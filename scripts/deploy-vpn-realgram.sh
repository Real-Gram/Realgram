...
# Deploy the repo-managed vpn.realgram.no nginx edge vhost safely.
# Mirrors XS227/SetaLink's scripts/deploy-vpn-edge.sh pattern for
# vpn-setalink, adapted for the realgram-branded twin edge on 5.249.252.88.
# This script intentionally does NOT modify /etc/nginx/nginx.conf or Xray
# config — the stream{} map entry (vpn.realgram.no -> 127.0.0.1:4434) and
# Xray inbounds are assumed already in place; this only manages the vhost file.
#
# Expected existing stream{} mapping:
#   vpn.realgram.no -> 127.0.0.1:4434
# Expected Xray loopback inbounds:
#   10000 WS, 10001 XHTTP, 10002 HTTPUpgrade

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SOURCE="$REPO_ROOT/infra/nginx/vpn-realgram.conf"
TARGET="/etc/nginx/sites-available/vpn-realgram"
ENABLED="/etc/nginx/sites-enabled/vpn-realgram"
CERT_DIR="/etc/letsencrypt/live/vpn.realgram.no"
...
if ! nginx -T 2>/dev/null | grep -Eq 'vpn\.realgram\.no[[:space:]]+127\.0\.0\.1:4434;'; then
  echo "ERROR: nginx stream{} does not map vpn.realgram.no to 127.0.0.1:4434" >&2
  exit 1
fi
...
health="$(curl --silent --show-error --fail --resolve vpn.realgram.no:4434:127.0.0.1 \
  https://vpn.realgram.no:4434/healthz)"
...
echo "OK: vpn.realgram.no edge listener restored on 127.0.0.1:4434"
