...
- [x] Inventory `.221`'s nginx+PHP-FPM setalink.no site (webroot `/var/www/setalink`) — **Inventory done 2026-08-31:** `sites-enabled/` on `.221` has `api.setalink.no`, `setalink-landing` (the main site), `calling-relay-ws`, `default` — plus, worth flagging, **`app.dadashi.no`** (Dr. Nasrin Dadashi's own unrelated site, also hosted on this box — needs its own decision if `.221` gets decommissioned, out of scope for this migration otherwise). PHP-FPM: single pool, `www.conf`.
  ...
  - **Superseded by a broader plan, same day:** Khabat's actual end goal is a **full-domain** redirect of `setalink.no` → `realgram.no` — but only once the app stops hardcoding `setalink.no/api.php` as `BASE_URL`. Prep sequence (mirrors the `vpn.realgram.no` parallel-cutover pattern from Work item B): stand up a new `api.realgram.no` serving the same API in parallel alongside the existing `setalink.no/api.php`, verify it, then flip `mobile-app/src/services/*`'s `BASE_URL` to it (app-release required, since it's compiled in — unlike the VPN node catalog/edge_host). **Until that ships: hold off on touching `setalink-landing`/`api.setalink.no` entirely — including the per-path marketing-page redirect approved above.** Khabat wants this done once, together, not as two separate redirect changes. Peer session is picking up the `api.realgram.no` prep on `.221`'s side.
