...
- [x] Inventory `.221`'s nginx+PHP-FPM setalink.no site (webroot `/var/www/setalink`) — **Inventory done 2026-08-31:** `sites-enabled/` on `.221` has `api.setalink.no`, `setalink-landing` (the main site), `calling-relay-ws`, `default` — plus, worth flagging, **`app.dadashi.no`** (Dr. Nasrin Dadashi's own unrelated site, also hosted on this box — needs its own decision if `.221` gets decommissioned, out of scope for this migration otherwise). PHP-FPM: single pool, `www.conf`.
  - **Khabat's decision (2026-08-31): the `setalink-landing` public site should redirect to `realgram.no`.** Scope this to the marketing/landing vhost only — **`api.setalink.no` (and/or `setalink.no/api.php`, whichever actually serves `remote-config`/`profile-bundle`/`v1.php` — confirm exact host first) must keep working unredirected**, since the app's compiled client hardcodes `https://setalink.no/api.php?...` and redirecting that domain would break the live server-pushed config/node-catalog for every existing user. Whoever has `.221` access: confirm which vhost is which before touching redirects, then add the `setalink-landing` → `realgram.no` redirect there, leaving the API vhost's `server_name` untouched.
